                        ...
                        let data = match &glyph.alpha_map {
                            ...
                            fonts::GlyphAlphaMap::Shared(data) => {
                                ...
                                target_pixel_buffer::TextureDataContainer::Shared {
                                    buffer: if glyph.subpixel {
                                        SharedBufferData::SubpixelMap {
                                            data: data.clone(),
                                            width: glyph.pixel_stride,
                                        }
                                    } else {
                                        SharedBufferData::AlphaMap {
                                            data: data.clone(),
                                            width: glyph.pixel_stride,
                                        }
                                    },
...
impl<T: ProcessScene> sharedparley::GlyphRenderer for SceneBuilder<'_, T> {
    ...
    fn draw_glyph_run(
        ...
        for positioned_glyph in glyphs_it {
            ...
            // 禁用亚像素偏移对齐：取整到整数像素，改善小字体清晰度
            let glyph_offset: euclid::Vector2D<i16, PhysicalPx> = euclid::Vector2D::new(
                (positioned_glyph.x as f32).round() as i16,
                ((positioned_glyph.y + y_offset.get() as f32).round()) as i16,
            );
            ...
            let target_rect: PhysicalRect = euclid::Rect::<f32, PhysicalPx>::new(
                (PhysicalPoint::from_lengths(PhysicalLength::new(0), -gl_y - glyph.height)
                    ...
                    + glyph_offset)
                    .cast()
                    + euclid::vec2(glyph.x.truncate() as f32, 0.0),
                glyph.size().cast(),
            )
            .cast()
            .transformed(self.rotation);
