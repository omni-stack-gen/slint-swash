...
pub(super) fn draw_texture_line(
    ...
    fn fetch_blend_pixel(
        ...
        match format {
            ...
            TexturePixelFormat::SubpixelMap => {
                // LCD subpixel rendering: data contains BGRA with per-channel alpha
                for pix in line_buffer {
                    let pos = pos(4).0;
                    // Subpixel data is already colored, just apply global alpha
                    let a = (data[pos + 3] as u16 * alpha as u16 / 255) as u8;
                    let c = PremultipliedRgbaColor {
                        alpha: a,
                        red: (data[pos + 2] as u16 * alpha as u16 / 255) as u8,
                        green: (data[pos + 1] as u16 * alpha as u16 / 255) as u8,
                        blue: (data[pos + 0] as u16 * alpha as u16 / 255) as u8,
                    };
                    pix.blend(c);
                }
            }
