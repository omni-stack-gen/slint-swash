...
pub enum SharedBufferData {
    ...
    SubpixelMap { data: Rc<[u8]>, width: u16 },
}
...
impl SharedBufferData {
    pub fn width(&self) -> usize {
        match self {
            ...
            SharedBufferData::SubpixelMap { width, .. } => *width as usize,
        }
    }
    #[allow(unused)]
    pub fn height(&self) -> usize {
        match self {
            ...
            SharedBufferData::SubpixelMap { data, width, .. } => data.len() / (*width as usize * 4),
        }
    }
}
...
impl SharedBufferCommand {
    pub fn as_texture(&self) -> SceneTexture<'_> {
        ...
        match &self.buffer {
            ...
            SharedBufferData::SubpixelMap { data, width } => SceneTexture {
                data: &data[start * 4..end * 4],
                pixel_stride: *width * 4,
                format: TexturePixelFormat::SubpixelMap,
                extra: self.extra,
            },
        }
    }
